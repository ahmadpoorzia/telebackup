...
static TexturedShape backgram_sphere, backgram_plane;
...
static GLuint backgram_sphere_texture, backgram_plane_texture;
...
JNIEXPORT void Java_org_backgram_messenger_Intro_onDrawFrame(JNIEnv *env, jclass class) {
    time_local += 0.016f;
    ...
    if (current_page == 0) {
        ...
        backgram_sphere.params.alpha = 1;
        ...
        backgram_sphere.params.alpha = alpha;
        backgram_sphere.params.scale = xyzMake(scale, scale, 1);

        backgram_plane.params.alpha = 1;
        ...
        backgram_plane.params.position = xyzMake(dx, dy, 0);

        float scale = (cosf(tt) + 1) * 0.5f;

        backgram_plane.params.scale = xyzMake(cosf(tt) * scale, scale, 1);
    ...
    if (current_page == 0) {
        rglNormalDraw();
        if (direct == 0) {
            backgram_sphere.params.alpha = t(0, 1, 0, duration_const * 0.8f, Linear);
            scale = 1;

            backgram_sphere.params.scale = xyzMake(scale, scale, 1);
            draw_textured_shape(&backgram_sphere, main_matrix, NORMAL);
            ...
            if (tt < D2R(125)) {
                glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
                draw_textured_shape(&backgram_plane, main_matrix, NORMAL_ONE);
            }
        }
    } else if (current_page == 1) {
        rglNormalDraw();
        if (direct == 1) {
            backgram_sphere.params.alpha = t(1, 0, 0, duration_const, Linear);
            draw_textured_shape(&backgram_sphere, main_matrix, NORMAL);
            ...
            backgram_plane.params.scale = xyzMake((float) cos(tt) * scale, scale, 1);
...
JNIEXPORT void Java_org_backgram_messenger_Intro_setScrollOffset(JNIEnv *env, jclass class, float a_offset) {
    scroll_offset = a_offset;
}

JNIEXPORT void Java_org_backgram_messenger_Intro_setPage(JNIEnv *env, jclass class, int32_t page) {
...
JNIEXPORT void Java_org_backgram_messenger_Intro_setDate(JNIEnv *env, jclass class, float a) {
    date = a;
}

JNIEXPORT void Java_org_backgram_messenger_Intro_setIcTextures(JNIEnv *env, jclass class, GLuint a_ic_bubble_dot, GLuint a_ic_bubble, GLuint a_ic_cam_lens, GLuint a_ic_cam, GLuint a_ic_pencil, GLuint a_ic_pin, GLuint a_ic_smile_eye, GLuint a_ic_smile, GLuint a_ic_videocam) {
...
JNIEXPORT void Java_org_backgram_messenger_Intro_setBackgramTextures(JNIEnv *env, jclass class, GLuint a_backgram_sphere, GLuint a_backgram_plane) {
    backgram_sphere_texture = a_backgram_sphere;
    backgram_plane_texture = a_backgram_plane;
}

JNIEXPORT void Java_org_backgram_messenger_Intro_setFastTextures(JNIEnv *env, jclass class, GLuint a_fast_body, GLuint a_fast_spiral, GLuint a_fast_arrow, GLuint a_fast_arrow_shadow) {
...
JNIEXPORT void Java_org_backgram_messenger_Intro_setFreeTextures(JNIEnv *env, jclass class, GLuint a_knot_up, GLuint a_knot_down) {
    free_knot_up_texture = a_knot_up;
    free_knot_down_texture = a_knot_down;
}

JNIEXPORT void Java_org_backgram_messenger_Intro_setPowerfulTextures(JNIEnv *env, jclass class, GLuint a_powerful_mask, GLuint a_powerful_star, GLuint a_powerful_infinity, GLuint a_powerful_infinity_white) {
...
JNIEXPORT void Java_org_backgram_messenger_Intro_setPrivateTextures(JNIEnv *env, jclass class, GLuint a_private_door, GLuint a_private_screw) {
    private_door_texture = a_private_door;
    private_screw_texture = a_private_screw;
}

JNIEXPORT void Java_org_backgram_messenger_Intro_onSurfaceCreated(JNIEnv *env, jclass class) {
    ...
    backgram_sphere = create_textured_rectangle(CSizeMake(150, 150), backgram_sphere_texture);
    backgram_plane = create_textured_rectangle(CSizeMake(82, 74), backgram_plane_texture);
    backgram_plane.params.anchor = xyzMake(6, -5, 0);
...
JNIEXPORT void Java_org_backgram_messenger_Intro_onSurfaceChanged(JNIEnv *env, jclass class, int32_t a_width_px, int32_t a_height_px, float a_scale_factor, int32_t a1) {
