...
public class FileLoader {
    ...
    public ConcurrentHashMap<String, FileLoadOperation> loadOperationPaths = new ConcurrentHashMap<>();
    ...
    public int getBackupDownloadCount(int dialog_id){
        int count = 0;
        for(FileLoadOperation op : loadOperationPaths.values()){
            if(op.isBackup && op.messageForBackup != null && op.messageForBackup.getDialogId() == dialog_id){
                count++;
            }
        }
        return count;
    ...
    public ArrayList<FileLoadOperation> getBackupDownloadOperations(int dialog_id){
        int count = 0;
        ArrayList<FileLoadOperation> res = new ArrayList<>();
        for(FileLoadOperation op : loadOperationPaths.values()){
            if(op.isBackup && op.messageForBackup != null && op.messageForBackup.getDialogId() == dialog_id){
                res.add(op);
            }
    ...
    public void loadFilePhoto(MessageObject msg, File path, TLRPC.PhotoSize photo, String ext) {
        loadFileWithPath(msg, path, null, null, photo.location, ext, photo.size, true, 0);
    }
    ...
    public void loadFileDocument(MessageObject msg, File path, TLRPC.Document document, boolean force) {
        loadFileWithPath(msg, path, document, null, null, null, 0, force, 0);
    }
    ...
    public void loadFileFile(MessageObject obg, File path, TLRPC.FileLocation location, String ext, int size, int cacheType) {
        loadFileWithPath(obg, path, null, null, location, ext, size, true, 0);
    }

    public void loadFilePath(File path, TLRPC.Video video, boolean force) {
        _LoadFile(path, video, null, null, null, null, 0, force, video != null && video.key != null);
    }

    public void loadFilePath(File path, TLRPC.PhotoSize photo, String ext, boolean cacheOnly) {
        _LoadFile(path, null, null, null, photo.location, ext, photo.size, false, cacheOnly || (photo != null && photo.size == 0 || photo.location.key != null));
    }

    public void loadFilePath(File path, TLRPC.Document document, boolean force, boolean cacheOnly) {
        _LoadFile(path, null, document, null, null, null, 0, force, cacheOnly || document != null && document.key != null);
    }

    public void loadFilePath(File path, TLRPC.Audio audio, boolean force) {
        _LoadFile(path, null, null, audio, null, null, 0, false, audio != null && audio.key != null);
    }

    public void loadFilePath(File path, TLRPC.FileLocation location, String ext, int size, boolean cacheOnly) {
        _LoadFile(path, null, null, null, location, ext, size, true, cacheOnly || size == 0 || (location != null && location.key != null));
    }

    public void _LoadFile(File path, final TLRPC.Video video, final TLRPC.Document document, final TLRPC.Audio audio, final TLRPC.FileLocation location, final String locationExt, final int locationSize, boolean _force, boolean _cacheOnly) {
        fileLoaderQueue.postRunnable(() -> {
            final boolean force = true;
            final boolean cacheOnly = false;
            String fileName = null;
            if (video != null) {
...
//                    checkDownloadQueue(audio, location, finalFileName);
    ...
    private void loadFileWithPath(MessageObject msg, File path, final TLRPC.Document document, final TLRPC.TL_webDocument webDocument, final TLRPC.FileLocation location, final String locationExt, final int locationSize, final boolean force, final int cacheType) {
        fileLoaderQueue.postRunnable(new Runnable() {
            @Override
            public void run() {
                String fileName = null;
                ...
                operation.isBackup = true;
                operation.messageForBackup = msg;
                loadOperationPaths.put(fileName, operation);
                int maxCount = force ? 3 : 1;
                if (type == MEDIA_DIR_AUDIO) {
                    if (currentAudioLoadOperationsCount < maxCount) {
