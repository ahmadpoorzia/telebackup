...
import org.backgram.messenger.voip.VoIPService;
...
import org.backgram.ui.ChatActivity;
import org.backgram.ui.Components.AlertsCreator;
import org.backgram.ui.ProfileActivity;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void loadAllMessages(final long dialog_id, final int classGuid, final int offset, final int max_id) {
        ...
        req.limit = 100;
        req.max_id = max_id;
        int reqId = ConnectionsManager.getInstance().sendRequest(req, (response, error) -> {
            if (response != null) {
...
//                }
                ...
                processLoadedAllMessages(res, dialog_id, classGuid, offset);
            }
        });
        ConnectionsManager.getInstance().bindRequestToGuid(reqId, classGuid);
    }

    public void processLoadedAllMessages(final TLRPC.messages_Messages messagesRes, final long dialog_id, final int classGuid, final int offset) {
        Utilities.stageQueue.postRunnable(() -> {
            ...
            AndroidUtilities.runOnUIThread(() -> {
                putUsers(messagesRes.users, false);
                putChats(messagesRes.chats, false);
                NotificationCenter.getInstance().postNotificationName(NotificationCenter.messagesDidLoadedBackup, dialog_id, objects, messagesRes.messages);
            });
            if(messagesRes.messages.size() == 0) {
                NotificationCenter.getInstance().postNotificationName(NotificationCenter.messagesDidLoadedCompletely);
            }
        });
    }
