package org.backgram.messenger.voip;
...
import org.backgram.messenger.StatsController;
...
import org.backgram.ui.VoIPActivity;
import org.backgram.ui.VoIPPermissionActivity;
...
public abstract class VoIPBaseService extends Service implements SensorEventListener, AudioManager.OnAudioFocusChangeListener, VoIPController.ConnectionStateListener{
	...
	public void onCreate() {
		...
		try {
			...
			cpuWakelock = ((PowerManager) getSystemService(POWER_SERVICE)).newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "backgram-voip");
			cpuWakelock.acquire();
	...
	protected void configureDeviceForCall() {
		...
		try{
			if(proximity!=null){
				proximityWakelock=((PowerManager) getSystemService(Context.POWER_SERVICE)).newWakeLock(PROXIMITY_SCREEN_OFF_WAKE_LOCK, "backgram-voip-prx");
