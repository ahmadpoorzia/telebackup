...
package org.backgram.messenger.browser;
...
import org.backgram.messenger.CustomTabsCopyReceiver;
...
import org.backgram.messenger.ShareBroadcastReceiver;
import org.backgram.messenger.support.customtabs.CustomTabsCallback;
import org.backgram.messenger.support.customtabs.CustomTabsClient;
import org.backgram.messenger.support.customtabs.CustomTabsIntent;
import org.backgram.messenger.support.customtabs.CustomTabsServiceConnection;
import org.backgram.messenger.support.customtabs.CustomTabsSession;
import org.backgram.messenger.support.customtabsclient.shared.CustomTabsHelper;
import org.backgram.messenger.support.customtabsclient.shared.ServiceConnection;
import org.backgram.messenger.support.customtabsclient.shared.ServiceConnectionCallback;
...
public class Browser {
    ...
    public static void openUrl(final Context context, final Uri uri, final boolean allowCustom, boolean tryTelegraph) {
        ...
        if (tryTelegraph) {
            try {
                ...
                if (host.equals("telegra.ph") || uri.toString().contains("backgram.org/faq")) {
    ...
    public static boolean isInternalUri(Uri uri, boolean forceBrowser[]) {
        ...
        if ("tg".equals(uri.getScheme())) {
            return true;
        } else if ("backgram.dog".equals(host)) {
        ...
        } else if ("backgram.me".equals(host) || "t.me".equals(host) || "telesco.pe".equals(host)) {
            String path = uri.getPath();
            if (path != null && path.length() > 1) {
                path = path.substring(1).toLowerCase();
                if (path.equals("iv")) {
